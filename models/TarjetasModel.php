...
class TarjetasModel {
    ...
    function __construct($args = []){
        ...
        $this->url = $args['url'] ?? '';
        $this->urlpedido = $args['urlpedido'] ?? '';
        $this->recomendado = $args['recomendado'] ?? 0;
    }
    ...
    public function insert($nombre, $idSello, $idEmisor, $tipo, $uso, $costoDeEmision, $costoPrimerAnio, $costoRenovacion, $costoAdicionales, $comisionComprasExterior, $pago, $cashback, $programaDePuntos, $limiteDeGastoMaximo, $interesCuotas, $cuentaBancaria, $aumentoDeLimitePorViaje, $costoEnvioEstadoDeCuenta, $envioEstadoDeCuentaEmail, $adelantoDeEfectivo, $informacionAdicional, $Imagen, $fechaDeCierre, $cambioFechaDeCierre, $telefonoParaExtravio, $Contactless, $reimpresionDePlastico, $reimpresionDePin, $reemplazoPorRoboExtravio, $url, $urlpedido, $recomendado){
        $envioEstadoDeCuentaEmail = $envioEstadoDeCuentaEmail == "0" ? 0 : 1;
        $query = "insert into tarjetas (url, nombre, id_sello, id_emisor, tipo, uso, costo_de_emision, costo_primer_anio, costo_renovacion, costo_adicionales, comision_compras_exterior, pago, cashback, programa_de_puntos, limite_de_gasto_maximo, interes_cuotas, cuenta_bancaria, costo_envio_estado_de_cuenta, envio_estado_de_cuenta_email, adelanto_de_efectivo, aumento_de_limite_por_viaje, telefono_para_extravio, contactless, reimpresion_de_plastico, reimpresion_de_pin, reemplazo_por_robo_extravio, cambio_fecha_de_cierre, fecha_de_cierre, imagen, informacion_adicional, urlpedido, recomendado) VALUES ('${url}','${nombre}', ${idSello}, ${idEmisor}, '${tipo}', '${uso}', ${costoDeEmision}, '${costoPrimerAnio}', '${costoRenovacion}', '${costoAdicionales}', '${comisionComprasExterior}', ${pago}, ${cashback}, ${programaDePuntos}, ${limiteDeGastoMaximo}, ${interesCuotas}, '${cuentaBancaria}', ${costoEnvioEstadoDeCuenta}, ${envioEstadoDeCuentaEmail}, ${adelantoDeEfectivo}, ${aumentoDeLimitePorViaje}, ${telefonoParaExtravio}, ${Contactless}, ${reimpresionDePlastico}, ${reimpresionDePin}, '${reemplazoPorRoboExtravio}' , '${cambioFechaDeCierre}' , '${fechaDeCierre}', '${Imagen}', ${informacionAdicional}, '${urlpedido}', ${recomendado})";
        var_dump($query);die;
        $result = $this->db->query($query);
        return $result;
    }
